...
## 项目简介
本仓库提供自动生成适用于爱快（iKuai）路由器的国内IPv4、IPv6地址分组及省份IP分组文件的工具，帮助用户快速快速配置路由策略，实现国内IP地址的精准管理。

## 功能说明
1. **自动获取国内IP段**：通过定期拉取公开的国内IPv4、IPv6及省份IP CIDR列表
...
- 生成的省份IP分组文件名为 `ikuai_cn_province_ipgroup.txt`
- 文件格式示例：`id=60 comment=1.0.1.0/24,1.0.2.0/23,... group_name=国内IPv4-1 addr_pool=1.0.1.0,1.0.2.0,...`
  - `id`：分组ID（IPv4从60开始递增，IPv6从70开始递增，省份IP可根据实际设置起始ID）
  - `group_name`：分组名称（按序号递增，如国内IPv4-1、国内IPv6-1、省份IP-1等）
  - `addr_pool`：包含的IP段列表（CIDR格式的网络部分）
  - `comment`：包含的完整CIDR列表（用于参考）

## 使用方法
1. 从仓库下载最新的 `ikuai_cn_ipv4group.txt`、`ikuai_cn_ipv6group.txt` 和 `ikuai_cn_province_ipgroup.txt` 文件
2. 登录爱快路由器管理后台
3. 进入 **网络设置 > IP分组** 页面
4. 分别导入IPv4、IPv6及省份IP分组文件或手动粘贴内容，完成对应IP分组配置

## 自动更新机制
通过GitHub Actions实现每日自动更新：
- 国内IPv4和IPv6分组更新时间：每天北京时间08:00（UTC时间00:00）
- 省份IP分组更新时间：每天北京时间02:00（UTC时间18:00）
